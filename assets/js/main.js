...
const renderContent = (apiData) =>
{
    ...
    // Werte aus apiData(Array) in variablen speichern
    let obtainDate = new Date();
    let userTimeDiff = obtainDate.getTimezoneOffset() / 60;
    let temperature = `${Math.round(apiData.main.temp - 273.15)}°C`;
    let cloudDescription = apiData.weather[0].description;
    ...
    let wind = `speed: ${apiData.wind.speed} m/s`;
    let humidity = `${apiData.main.humidity} %`;

    // Bearbeitung der sunrise / sunset daten
    let sunriseCurrentTime = new Date((apiData.sys.sunrise) * 1000);
    ...
    let sunsetCurrentTime = new Date((apiData.sys.sunset) * 1000);
